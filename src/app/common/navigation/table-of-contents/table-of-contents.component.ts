...
import {NavigationFocusService} from '../../../_services/navigation-focus.service';
...
export class TableOfContentsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    Promise.resolve().then(() => {
      this._scrollContainer = this.container ?
        this._document.querySelectorAll(this.container)[0] : window.document.querySelectorAll('.content-wrapper')[0];
    ...
    this.updateTableOfContents("Guide Content", <HTMLElement>this._document.querySelectorAll(this.content)[0]);
  }
  ...
  updateTableOfContents(sectionName: string, docViewerContent: HTMLElement, sectionIndex = 0) {
      this.addHeaders(sectionName, docViewerContent, sectionIndex);
      this.updateScrollPosition();
  }
  ...
  private getScrollOffset(): number | void {
    const top = 100;//this._element.nativeElement.getBoundingClientRect();
    if (typeof this._scrollContainer !== 'undefined' && typeof this._scrollContainer.scrollTop !== 'undefined') {
      return this._scrollContainer.scrollTop + top;
    } else if (typeof this._scrollContainer !== 'undefined' && typeof this._scrollContainer.pageYOffset !== 'undefined') {
